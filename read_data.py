...
def read_from_db(
    ...
    try:
        ...
        result = []
        for _, timestamp, hundredsOfSecond, rowValue in rows:
            _hundredsOfSecond = int(hundredsOfSecond)
            _rowValue = int(rowValue)
            try:
                _timestamp = datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S.%f")
            except ValueError:
                _timestamp = datetime.strptime(timestamp + ".000", "%Y-%m-%d %H:%M:%S.%f")
